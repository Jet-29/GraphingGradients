...
namespace Spyder {
	Pipeline::Pipeline(Device &device, const std::vector<uint32_t> &vertShaderArray, const std::vector<uint32_t> &fragShaderArray, const PipelineConfigInfo &configInfo) : device{device} {
	...
	void Pipeline::defaultPipelineConfigInfo(PipelineConfigInfo &configInfo) {
		...
		configInfo.bindingDescriptions = Drawable::Vertex::getBindingDescriptions();
		configInfo.attributeDescriptions = Drawable::Vertex::getAttributeDescriptions();
	}
	...
	void Pipeline::createGraphicsPipeline(const std::vector<uint32_t> &vertShaderArray, const std::vector<uint32_t> &fragShaderArray, const PipelineConfigInfo &configInfo) {
		...
		assert(configInfo.renderPass != VK_NULL_HANDLE && "Cannot create graphics pipeline, no pipeline renderpass provided");

		createShaderModule(vertShaderArray, &vertShaderModule);
		createShaderModule(fragShaderArray, &fragShaderModule);
