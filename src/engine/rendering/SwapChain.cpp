...
namespace Spyder {
	...
	SwapChain::SwapChain(Device &device, VkExtent2D windowExtent, std::shared_ptr<SwapChain> previous) : device{device}, windowExtent{windowExtent}, oldSwapChain{std::move(previous)} {
		init();
	...
	VkResult SwapChain::acquireNextImage(uint32_t *imageIndex) {
		...
		return result; // must be a not signaled semaphore
	}
	...
	bool SwapChain::compareSwapFormats(const SwapChain &swapChainToCompare) const {
		return swapChainToCompare.swapChainDepthFormat == swapChainDepthFormat && swapChainToCompare.swapChainImageFormat == swapChainImageFormat;
	}
	...
	void SwapChain::createSwapChain() {
		...
		// allowed to create a swap chain with more. so query the final number of
		// images with vkGetSwapchainImagesKHR, then resize the container and call it again to
	...
	void SwapChain::createDepthResources() {
		...
		VkExtent2D currentSwapChainExtent = getSwapChainExtent();
		...
		for (int i = 0; i < depthImages.size(); i++) {
			...
			imageInfo.extent.width = currentSwapChainExtent.width;
			imageInfo.extent.height = currentSwapChainExtent.height;
	...
	void SwapChain::createFramebuffers() {
		...
		for (size_t i = 0; i < imageCount(); i++) {
			...
			framebufferInfo.width = currentSwapChainExtent.width;
			framebufferInfo.height = currentSwapChainExtent.height;
			framebufferInfo.layers = 1;

			if (vkCreateFramebuffer(
					device.device(), &framebufferInfo, nullptr, &swapChainFramebuffers[i]) != VK_SUCCESS) {
